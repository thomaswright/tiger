...
import { TODO_STATUSES, type TodoStatus } from "./shared/domain";
  ...
  return (
    <DropdownMenu.Root modal={false}>
      <DropdownMenu.Trigger asChild>
        <button
          className={`flex h-7 min-w-20 items-center rounded px-2 text-2xs font-medium outline-none transition-shadow hover:ring-1 hover:ring-current focus-visible:ring-2 focus-visible:ring-blue-500 ${STATUS_COLORS[value]}`}
          ...
          >
            {TODO_STATUSES.map((status) => (
              <DropdownMenu.RadioItem
                key={status}
                value={status}
                className={`flex h-8 cursor-default select-none items-center rounded border border-transparent px-2 text-2xs font-medium outline-none transition-shadow data-[state=checked]:border-current data-[highlighted]:ring-2 data-[highlighted]:ring-blue-500 ${STATUS_COLORS[status]}`}
              >
